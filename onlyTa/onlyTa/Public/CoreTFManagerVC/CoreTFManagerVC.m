...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
  //  NSLog(@" shouldChangeCharactersInRange");
    InputType inputMode=0;
    for (TFModel *tfModel in self.tfModels) {
        if(tfModel.textField==textField){
            inputMode=tfModel.inputMode;
            break;
        }
    }
    if (inputMode == InputType_Phone){//手机号码,只能输入11位数字
        NSCharacterSet *cs;
        ...
        return YES;
    }else if(inputMode == InputType_Age){ //年龄,只能输入2位数字
        NSCharacterSet *cs;
        cs = [[NSCharacterSet characterSetWithCharactersInString:@"1234567890"] invertedSet];
        NSString *filtered = [[string componentsSeparatedByCharactersInSet:cs] componentsJoinedByString:@""];
        BOOL basicTest = [string isEqualToString:filtered];
        ...
        return YES;
    }else if(inputMode == InputType_PWD){ //密码,长度16
        NSCharacterSet *cs;
