...
@interface BaseViewController (){
    UIImage *naviBarBgImage;
    UIImage *shadowImage;// 导航栏下面的一条线
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.navigationController.navigationBarHidden=_needHideNaviBar;
    UINavigationBar *bar = self.navigationController.navigationBar;
    if(naviBarBgImage==nil){
        naviBarBgImage=[self createImageWithColor:_naviBarBgColor];
    }
    [bar setBackgroundImage:naviBarBgImage forBarMetrics:UIBarMetricsDefault];
    if(shadowImage==nil){
        shadowImage=[self createImageWithColor:_naviBarShadowColor widthHeight:0.5];
    }
...
- (void)setNavigationTitle:(NSString *)title withColor:(UIColor*)titleColor{
...
-(void)push:(UIViewController *)vc needHideBottomBar:(BOOL)hide animated:(BOOL)animated{
    vc.hidesBottomBarWhenPushed=hide;
    [self.navigationController pushViewController:vc animated:animated];
}

-(void)showToastDialog:(NSString *)title{
    [ToastUtils showToastDialog:title];
}
-(void)showProgress:(NSString *)title{
    [ToastUtils showProgress:title];
}
-(void)showLodingProgress{
    [ToastUtils showLodingProgress];
}

-(void)stopProgress{
    [ToastUtils stopProgress];
}
/**
 点击空白回收键盘
 */
